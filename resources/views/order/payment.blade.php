            ...
            <section id="your-order" class="accordion flex flex-col rounded-[20px] p-4 pb-5 gap-5 mx-4 bg-white overflow-hidden transition-all duration-300 has-[:checked]:!h-[66px]">
                ...
                <div class="flex items-center gap-[14px]">
                    <div class="flex shrink-0 w-20 h-20 rounded-[20px] p-1 overflow-hidden">
                        <img src="{{ Storage::url($product->photos()->latest()->first()->photo) }}" class="w-full h-full object-contain" alt="">
                    </div>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <p class="font-bold">{{ $product->brand->name }}</p>
                </div>
                <div class="flex items-center justify-between">
                ...
                <div class="flex items-center justify-between">
                    ...
                    <p class="font-bold">{{ $orderData['product_size'] }}</p>
                </div>
            </section>
            ...
            <section id="send-payment-to" class="accordion flex flex-col rounded-[20px] p-4 pb-5 gap-5 mx-4 bg-white overflow-hidden transition-all duration-300 has-[:checked]:!h-[66px]">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex flex-col gap-[2px]">
                        <p class="font-semibold flex items-center">TKD Sport Indonesia <img src="{{asset('assets/images/icons/verify.svg')}}" class="ml-1" alt="icon"></p>
                        <p>8008129839</p>
                    </div>
                </div>
                <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="group w-full rounded-full px-[14px] flex items-center ring-1 ring-[#090917] gap-[10px] relative transition-all duration-300 focus-within:ring-2 focus-within:ring-[#FFC700]">
                        ...
                        <button type="button" id="upload-btn" class="appearance-none outline-none w-full py-[14px] text-left text-sm overflow-hidden text-[#878785]">
                            Add an attachment
                        </button>
                        
                        <input type="file" name="proof" id="proof" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer" required>
                    </div>
                </div>
                <hr class="border-[#EAEAED]">
...
@endif
        ...
        <script>
            document.getElementById('proof').addEventListener('change', function(event) {
                let fileName = event.target.files[0] ? event.target.files[0].name : "Add an attachment";
